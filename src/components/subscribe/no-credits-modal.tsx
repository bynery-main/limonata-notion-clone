...
const NoCreditsModal: React.FC<NoCreditsModalProps> = ({
  ...
  const getNoCreditsText = () => {
    const resetTime = "0:00 -2GMT";
    if (subscriptionStatus === "pro") {
      return `You've reached your credit limit. You will get 1000 credits at ${resetTime}.`;
    } else {
      return `You've reached your credit limit. You can upgrade to Pro or wait until ${resetTime} when your credits will reset.`;
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gradient-to-br from-green-50 via-blue-50 to-pink-50 dark:bg-neutral-800 rounded-lg p-6 w-200">
        <h2 className="flex text-xl font-bold mb-4 justify-center">Out of Credits</h2>
                {/* 
        ...
        */}
                <p className="mb-4 text-center">{getNoCreditsText()}</p>
        {/* Render the PricingPage component only if the user is a free or active_pending_cancellation */}
        {subscriptionStatus === "free" && <PricingPage />}
        {subscriptionStatus === "active_pending_cancellation" && <PricingPage />}
        ...
        <div className="flex justify-center items-center">
          ...
          <Button
            onClick={onClose}
            variant="outline"
            className="mt-2 ml-2 w-full hover:bg-black hover:text-white w-40 p-2 rounded-lg border-2 border-black"
          >
            Close
          </Button>
        </div>
