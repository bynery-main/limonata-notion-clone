...
import { Home, Folder, FileText, BookOpen, Layout, HelpCircle } from 'lucide-react';
    ...
    const fetchBreadcrumbs = async () => {
      ...
      if (pathSegments.length > 1) {
        ...
        if (pathSegments.length === 3) {
          ...
          const folderDoc = await getDoc(doc(db, `workspaces/${workspaceId}/folders`, folderId));
          ...
          if (pathSegments.length > 3) {
            ...
            const itemDoc = await getDoc(doc(db, `workspaces/${workspaceId}/${collection}`, itemId));
            ...
            if (itemType === 'folders' && pathSegments.length > 4) {
              const noteId = pathSegments[4];
              const noteDoc = await getDoc(doc(db, `workspaces/${workspaceId}/folders/${itemId}/notes`, noteId));
              const noteName = noteDoc.exists() ? noteDoc.data().name : 'Note';
              console.log('Note Name:', noteName);

              items.push({
                href: `/dashboard/${workspaceId}/${itemType}/${itemId}/${noteId}`,
                label: noteName,
